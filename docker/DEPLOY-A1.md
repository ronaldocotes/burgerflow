# BurgerFlow — deploy na Oracle A1 (backend-first)
...
- `backend/src/main/resources/application-prod.yml` — perfil `prod`: **JWT secret
  obrigatório** (sem fallback de dev → app não sobe sem `BF_JWT_SECRET`), rate-limit
...
1. `git clone`/`pull` do repo (deploy key) em `/home/ubuntu/burgerflow` (ou no
...
2. `cd docker && cp env.prod.example .env.prod` e preencher com `openssl rand`:
   ...
   - `BF_DB_PASSWORD` / `SPRING_REDIS_PASSWORD` = `openssl rand -base64 24`
   - `SITE_ADDRESS` = `burgerflow.163-176-212-54.sslip.io` (ou domínio real).
...
5. **Migrations:** o Flyway do banco de **controle** roda no boot do app; cada
   **tenant** migra no 1º acesso. Para migrar bancos já existentes fora de banda:
   ...
   ../scripts/apply-migrations.sh \
     "jdbc:postgresql://localhost:5432/burgerflow_control?user=U&password=P" \
