...
# Executa a Fase 1 da auditoria frontend em QA local:
# - sobe backend em 8088 apontado para o Postgres QA;
# - garante e popula o tenant audit;
# - sobe frontend em 3001 apontado para o backend QA;
# - roda docs/auditorias/2026-06-29-menuflow-critical-audit.cjs;
# - encerra os processos iniciados por este script.

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
LOG_DIR="$ROOT_DIR/.codex-run"
BACKEND_LOG="$LOG_DIR/audit-backend.log"
FRONTEND_LOG="$LOG_DIR/audit-frontend.log"
...
node "$ROOT_DIR/docs/auditorias/2026-06-29-menuflow-critical-audit.cjs"

echo "Auditoria frontend local concluida."
