...
class GeocodingServiceTest {
    ...
    fun `geocode caches by normalized address and hits remote only once`() {
        ...
        Mockito.doReturn(LatLng(1.0, 2.0)).`when`(svc).fetchLatLng(Mockito.anyString())
        ...
        Mockito.verify(svc, Mockito.times(1)).fetchLatLng(Mockito.anyString())
    }
    ...
    fun `blank api key returns null and never calls remote`() {
        ...
        Mockito.verify(svc, Mockito.never()).fetchLatLng(Mockito.anyString())
    }
}
