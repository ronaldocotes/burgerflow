...
class LoginRateLimitTest @Autowired constructor(
    ...
    companion object {
        ...
        fun enableRateLimit(registry: DynamicPropertyRegistry) {
            registry.add("menuflow.rate-limit.login.enabled") { "true" }
            registry.add("menuflow.rate-limit.login.backend") { "memory" }
            registry.add("menuflow.rate-limit.login.capacity") { "5" }
            registry.add("menuflow.rate-limit.login.refill-period-seconds") { "60" }
        }
    }
