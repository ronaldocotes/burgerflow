...
class TenantFlywayMigrationTest @Autowired constructor(
    ...
    fun `provisioning a new tenant applies V1 and records the ledger`() {
        ...
        // V1..V9 applied exactly once each in the tenant's history table.
        assertEquals(9, countSchemaVersionRows(slug), "tenant schema_version must have exactly nine applied migrations (V1..V9)")
    ...
    fun `re-provisioning the same tenant is a no-op for the history table`() {
        ...
        assertEquals(9, countSchemaVersionRows(slug), "re-running migrate must NOT add duplicate rows")
