...
class TenantFlywayMigrationTest @Autowired constructor(
    ...
    fun `provisioning a new tenant applies V1 and records the ledger`() {
        ...
        // V1..V19 applied exactly once each in the tenant's history table.
        assertEquals(19, countSchemaVersionRows(slug), "tenant schema_version must have exactly nineteen applied migrations (V1..V19)")
    ...
    fun `re-provisioning the same tenant is a no-op for the history table`() {
        ...
        assertEquals(19, countSchemaVersionRows(slug), "re-running migrate must NOT add duplicate rows")
