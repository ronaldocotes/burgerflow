...
class TenantFlywayMigrationTest @Autowired constructor(
    ...
    fun `provisioning a new tenant applies V1 and records the ledger`() {
        ...
        assertEquals(10, countSchemaVersionRows(slug), "tenant schema_version must have exactly ten applied migrations (V1..V10)")
    ...
    fun `re-provisioning the same tenant is a no-op for the history table`() {
        ...
        assertEquals(10, countSchemaVersionRows(slug), "re-running migrate must NOT add duplicate rows")
