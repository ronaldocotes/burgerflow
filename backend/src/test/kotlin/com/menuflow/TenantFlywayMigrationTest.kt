...
class TenantFlywayMigrationTest @Autowired constructor(
    ...
    fun `provisioning a new tenant applies V1 and records the ledger`() {
        ...
        // V1..V40 applied exactly once each in the tenant's history table.
        assertEquals(40, countSchemaVersionRows(slug), "tenant schema_version must have exactly forty applied migrations (V1..V40)")
    ...
    fun `re-provisioning the same tenant is a no-op for the history table`() {
        ...
        assertEquals(40, countSchemaVersionRows(slug), "re-running migrate must NOT add duplicate rows")
