...
class TenantFlywayMigrationTest @Autowired constructor(
    ...
    fun `provisioning a new tenant applies V1 and records the ledger`() {
        ...
        // V1..V50 applied exactly once each in the tenant's history table.
        assertEquals(50, countSchemaVersionRows(slug), "tenant schema_version must have exactly fifty applied migrations (V1..V50)")
    ...
    fun `re-provisioning the same tenant is a no-op for the history table`() {
        ...
        assertEquals(50, countSchemaVersionRows(slug), "re-running migrate must NOT add duplicate rows")
