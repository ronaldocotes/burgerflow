...
class TenantFlywayMigrationTest @Autowired constructor(
    ...
    fun `provisioning a new tenant applies V1 and records the ledger`() {
        ...
        assertEquals(3, countSchemaVersionRows(slug), "tenant schema_version must have exactly three applied migrations (V1 + V2 + V3)")
    ...
    fun `re-provisioning the same tenant is a no-op for the history table`() {
        ...
        assertEquals(3, countSchemaVersionRows(slug), "re-running migrate must NOT add duplicate rows")
