...
| 2026-06-30 | Executar Fase 6.3 - Verificador de migrations por tenant | Concluida | `scripts/check-tenant-migrations.sh`, `docs/auditorias/2026-06-30-relatorio-fase-6-ci-performance-producao.md` | Script read-only criado e validado no Postgres QA e no A1 real. QA: `audit` e `demo` em V31, sem drift. A1: tenant ativo `demo` em V31, `tenants_with_drift=0`. Script corrigido para usar `docker exec menuflow-postgres psql` quando `psql` nao existe no host. |
...
A Fase 6 foi iniciada com foco em CI, performance e producao. O script unico de gates locais foi criado, o smoke seguro do dominio real foi adicionado, o deploy A1 foi reconciliado em artefatos versionados e a checagem read-only de migrations por tenant foi validada no QA e no A1 real. Proxima frente recomendada: alinhar/documentar o cron pull-deploy real do host antes de decidir qualquer reativacao de CD automatico no GitHub Actions.
